...
Function HMAC(atext,akey)
	init_vals()

	dim text, key
	
	' 转换为 UTF8 编码 汉字为3个字节，字幕为1个字节
	' 由于非 Unicdoe 的2个字节，所以 Len Mid Asc 等函数必须改为 LenB MidB AscB
	text = GB2UTF( atext)	
	key = GB2UTF( akey )
	...
	For x=0 To LenB(hkey)-1
	     ipad(x) = ipad(x) Xor AscB(CStr(MidB(hkey,x+1,1)))
	     opad(x) = opad(x) Xor AscB(CStr(MidB(hkey,x+1,1)))
	     idata(x) = ipad(x) And &hFF
	     odata(x) = opad(x) And &hFF
	Next

    For x=0 To LenB(text)-1
         idata(64+x) = AscB(CStr(MidB(text,x+1,1))) And &hFF
    Next 
